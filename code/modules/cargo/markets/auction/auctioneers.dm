...
/datum/auctioneer/proc/start_bidding()
	...
	to_chat(world, "[name]'s starting bid is [current_bid]")
...
/datum/auctioneer/proc/handle_bidding()
	...
	if(highest_bidder == src)
		...
		COOLDOWN_START(src, bid_cd, bidding_delay) //restart their delay, but without randomness
...
/datum/auctioneer/proc/generate_new_bid(current_auction_price)
	var/bid_proposal = round(abs(gaussian(0, SSauction.minimum_bid + (SSauction.minimum_bid * (rand(1,10)/10) )))) //Random just adds a neat little
...
/datum/auctioneer/random/also_random
